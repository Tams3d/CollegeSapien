...
interface User { id: string; name: string; email: string; role: string; collegeId?: string; collegeName?: string }
...
const ambassadors = computed(() => users.value.filter((u) => u.role === 'ambassador'))
  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Can upload freely; no moderation powers</p>
      </div>
      <button
        v-if="authStore.isSuperAdmin"
        class="px-3 py-2 text-sm bg-yellow-400 text-gray-900 font-medium rounded-lg hover:bg-yellow-500 transition-colors"
        @click="selectedUser = null; selectedCollegeId = ''; showPromoteModal = true"
      >
        Promote User
      </button>
    </div>
    ...
    <div v-else-if="ambassadors.length === 0" class="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-500 text-sm">
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="text-xs text-gray-500 border-b border-gray-100">
            ...
            <th v-if="authStore.isSuperAdmin" class="px-4 py-3 text-left font-medium">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="user in ambassadors" :key="user.id" class="border-b border-gray-50">
    ...
    <!-- Promote modal -->
    <div v-if="showPromoteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
        ...
        <template v-if="!selectedUser">
          <p class="text-xs text-gray-500 mb-2">Search users in the <NuxtLink to="/users" class="underline">Users page</NuxtLink> and click on a user to assign the ambassador role from their profile.</p>
        </template>

        <template v-else>
          <p class="text-sm text-gray-600 mb-3">{{ selectedUser.email }}</p>
          <p class="text-xs text-gray-500 mb-4">Ambassadors can upload freely and do not require college scope.</p>
          <div class="flex justify-end gap-2">
            <button class="px-4 py-2 text-sm border border-gray-300 rounded-lg" @click="showPromoteModal = false">Close</button>
            <button
